...
if not fileReadable (outfilename$)
	writeFileLine: outfilename$, "speaker",tab$, "vowel",tab$, "phone",tab$, "time",tab$, "duration",tab$, "F1",tab$, "F2",tab$, "F3",tab$, "ceiling",tab$, "F1Bk",tab$, "F2Bk",tab$, "F3Bk",tab$, "filename",tab$, "status"
...
# Выборочные средние первой-третьей форманты
   mf1 = 0 
   mf2 = 0 
   mf3 = 0 
   sum1 = 0
   sum2 = 0
   sum3 = 0
# Исправленная выборочная дисперсия первой-третьей форманты
   vf1 = 0 
   vf2 = 0 
   vf3 = 0 
   sumdev1 = 0
...
#Считаем средние выборочные значения формант
   for k to n
      sum1 = sum1 + f1b[k]
      sum2 = sum2 + f2b[k]
      sum3 = sum3 + f3b[k]
...
#Считаем исправленные выборочные дисперсии
   for k to n
      sumdev1 = sumdev1 + (f1b[k]-mf1)^2
      sumdev2 = sumdev2 + (f2b[k]-mf2)^2
      sumdev3 = sumdev3 + (f3b[k]-mf3)^2
   endfor
   vf1 = if n>1 then sumdev1/(n - 1) else 0 fi
   vf2 = if n>1 then sumdev2/(n - 1) else 0 fi
   vf3 = if n>1 then sumdev3/(n - 1) else 0 fi
...
for k to n
   ...
   appendFileLine: outfilename$, speaker$,tab$, .vowel$,tab$, .phone$,tab$, fixed$(.start_time,3),tab$, 
	...
	... fixed$(result_f1b[k],2),tab$, fixed$(result_f2b[k],2),tab$, fixed$(result_f3b[k],2),tab$, s$,tab$, "Auto"
endfor
